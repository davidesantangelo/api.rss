...
class SearchController < BaseController
  ...
  def feeds
    ...
    feeds = Feed.pagy_search(params[:q], boost_by: %i[entries_count rank], fields: fields).results
  ...
  def build_order
    ...
    order[:published_at] = entries_params[:order] if entries_params[:order].present?
  ...
  def build_filters
    ...
    range[:gte] = Time.parse(entries_params[:from]) if entries_params[:from].present?
    range[:lte] = Time.parse(entries_params[:to]) if entries_params[:to].present?

    filters[:feed_id] = entries_params[:feed_id] if entries_params[:feed_id].present?
    filters[:published_at] = range
